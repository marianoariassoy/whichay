...
const Stadistics = () => {
  ...
  return (
    <section className="relative bg-center bg-cover" id="stadistics">
      ...
      <div className="container px-14 py-48 m-auto max-w-7xl grid grid-cols-4 gap-24 text-white text-center items-start">
        {dataStadistics.map((item, index) => (
          <article className="font-semibold flex flex-col justify-center gap-6" key={index}>
            <h3>{lan === "es" ? item.title : item.title_en}</h3>
            <hr className="border-y-2" />
            <h2 className="text-4xl font-bold my-4">{item.subtitle}</h2>
            <hr className="border-y-2" />
